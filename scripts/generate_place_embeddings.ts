 ...
 *
 * This script batch processes all places in the boston_places collection
 * and generates 768-dimensional embeddings using Gemini text-embedding-004.
 *
 * Usage:
 * 1. Ensure GEMINI_API_KEY and MONGODB_URI are set in .env
 * 2. Run: npx ts-node scripts/generate_place_embeddings.ts
 */
 ...
 */
function createSemanticText(place: Place): string {
  const parts: string[] = [];
