...
// Unified search result that can be place, event, landmark, or reddit post
export type UnifiedSearchResult =
	| (VectorSearchResult & { type: "place" })
	| EventSearchResult
	| RedditSearchResult
...
export function getSentimentBadge(
	place: VectorSearchResult,
): PlaceWithSentiment["sentimentBadge"] | null {
	if (!place.reddit_mentions || place.reddit_mentions.length === 0) {
		return null;
	}

	const avgSentiment =
		place.reddit_mentions.reduce((sum, m) => sum + m.sentiment, 0) /
		place.reddit_mentions.length;
	const totalScore = place.reddit_mentions.reduce(
		(sum, m) => sum + m.score,
		0,
	);
