...
export default function ItineraryPanel({
  ...
  const voiceOnboarding = useVoiceOnboarding({
    ...
    onInterestsSelected: (interests: string[]) => {
      ...
      interests.forEach(interest => {
        if (interest === 'events') {
        ...
        } else if (interest === 'restaurants' || interest === 'places') {
          console.log('[ItineraryPanel] Enabling PLACES layer (restaurants)');
          ...
          setTimeout(() => discoveryPaneRef.current?.triggerSearch('hidden gem local secret'), 100);
        }
      });
    ...
    onSearchQuery: (query: string) => {
      ...
      setTimeout(() => discoveryPaneRef.current?.triggerSearch(query), 100);
    },
    onNavigate: (destination) => {
      console.log('[ItineraryPanel] Voice navigation to:', destination);
      if (destination === 'map') {
        // The map is always visible on the right - just acknowledge
        console.log('[ItineraryPanel] Map is visible on the right panel');
    ...
    onAddToItinerary: (placeName) => {
      console.log('[ItineraryPanel] Add to itinerary (by name):', placeName);
      // Search for the place and add the first result
      setSearchQuery(placeName);
      setCurrentStep(3);
      setTimeout(() => discoveryPaneRef.current?.triggerSearch(placeName), 100);
    },
  ...
  useEffect(() => {
    console.log('[Voice] voiceOnboardingActive:', voiceOnboardingActive, 'voiceOnboarding.isActive:', voiceOnboarding.isActive);
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-shrink-0">
                {voiceOnboarding.voiceState === 'listening' && (
                  <div className="w-12 h-12 rounded-full bg-cyan-500/20 border-2 border-cyan-500 flex items-center justify-center animate-pulse">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-cyan-500">
                ...
                {voiceOnboarding.voiceState === 'speaking' && (
                  <div className="w-12 h-12 rounded-full bg-green-500/20 border-2 border-green-500 flex items-center justify-center">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-green-500 animate-pulse">
                      <path d="M2 10v3" /><path d="M6 6v11" /><path d="M10 3v18" /><path d="M14 8v7" /><path d="M18 5v13" /><path d="M22 10v3" />
                    </svg>
                  </div>
                )}
                {voiceOnboarding.voiceState === 'processing' && (
                  <div className="w-12 h-12 rounded-full bg-yellow-500/20 border-2 border-yellow-500 flex items-center justify-center">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-yellow-500 animate-spin">
              ...
              <div className="flex-1 min-w-0">
                {voiceOnboarding.currentQuestion ? (
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-xs mt-1">
                      Try: "zoom into Chinatown", "show me bars", "indian food"
                    </p>
                  </div>
                )}
                ...
                {voiceOnboarding.error && (
                  <p className="text-red-400 text-xs mt-1">{voiceOnboarding.error}</p>
                )}
              </div>
